...
**文件位置**: `python/sglang/srt/lora/lora.py`
...
**文件位置**: `python/sglang/srt/lora/mem_pool.py`
...
**文件位置**: `python/sglang/srt/lora/lora_manager.py`
...
**文件位置**: `python/sglang/srt/lora/lora_registry.py`
...
| 组件 | 关键函数 | 文件位置 |
|------|---------|----------|
| LoRA 适配器 | `initialize_weights()` | `python/sglang/srt/lora/lora.py` |
| QKV 标准化 | `normalize_qkv_proj()` | `python/sglang/srt/lora/lora.py` |
| 内存池 | `prepare_lora_batch()` | `python/sglang/srt/lora/mem_pool.py` |
| 驱逐策略 | `get_available_buffer_slot()` | `python/sglang/srt/lora/mem_pool.py` |
| 批次准备 | `prepare_lora_batch()` | `python/sglang/srt/lora/lora_manager.py` |
| Layer 封装 | `apply_lora()` | `python/sglang/srt/lora/layers.py` |
| Triton Kernel | `run_lora_a_sgemm()` | `python/sglang/srt/lora/backend/triton_backend.py` |
| 注册表 | `acquire()` | `python/sglang/srt/lora/lora_registry.py` |
...
4. **量化模型兼容性**: 不同量化格式和后端组合仍需逐项回归
   - 当前状态: 主干能力可用，建议按模型与后端组合做专项验证
